...
export const PERFORMANCE_EXTERNAL_METRIC_CAPTURE_SCHEMA_VERSION = 4;
export const PERFORMANCE_EXTERNAL_METRIC_CAPTURE_INDEX_SCHEMA_VERSION = 4;
...
function body(input) {
  ...
  if (input.captureKind !== 'external-metric') fail('captureKind must be external-metric');
  const join = validatePerformanceRunJoin(input.join, { label: 'external metric run join' });
  ...
  const rawKinds = validatePerformanceRunRawKinds(input.rawKinds, {
    ...
    join,
    label: 'external metric rawKinds'
  });
  return {
...
function relativePath(capture) {
  return `${PERFORMANCE_EXTERNAL_METRIC_CAPTURE_DIRECTORY}/${capture.checksum}.json`;
}
...
export async function readPerformanceExternalMetricCaptures({ outputDirectory } = {}) {
  if (typeof outputDirectory !== 'string' || outputDirectory.length === 0) fail('outputDirectory must be a nonempty string');
  const directory = path.join(outputDirectory, PERFORMANCE_EXTERNAL_METRIC_CAPTURE_DIRECTORY);
  let entries;
  try {
  ...
  for (const name of names) {
    const capture = validatePerformanceExternalMetricCapture(JSON.parse(await fs.readFile(path.join(directory, name), 'utf8')));
    if (name !== `${capture.checksum}.json`) fail('capture filename does not match checksum');
    captures.push(freeze({ relativePath: `${PERFORMANCE_EXTERNAL_METRIC_CAPTURE_DIRECTORY}/${name}`, capture }));
  }
