...
function extractViteAliasKeys() { return extractAliasKeysFromConfigSource(readProjectText('vite.config.js'), 'vite.config.js'); }
function extractVitestAliasKeys() { return extractAliasKeysFromConfigSource(readProjectText('vitest.config.js'), 'vitest.config.js'); }
...
function createSettingsStartupPreferenceEventOwnershipCheck(settingsManifest, eventManifest, sourceText) { const eventPayloadByValue = new Map((eventManifest.scopes.find((entry) => entry.scope === 'renderer')?.events || []).map((event) => [event.value, event.payload])), payloadByType = { boolean: 'boolean', enum: 'string', number: 'number', string: 'string' }, eventFailures = (settingsManifest.definitions || []).filter((definition) => typeof definition.event === 'string' && definition.event.trim()).flatMap((definition) => { const actual = eventPayloadByValue.get(definition.event.trim()), expected = payloadByType[definition.type]; return !actual ? [`${definition.name} missing renderer event ${definition.event}`] : actual !== expected ? [`${definition.name} payload ${actual} !== ${expected}`] : []; }), manualReferences = (settingsManifest.definitions || []).filter((definition) => definition.startupPreference === true && typeof definition.event === 'string' && definition.event.trim()).map((definition) => { const [domain, name] = definition.event.trim().split(':'); return domain && name ? `${definition.name} EventChannels.${domain.toUpperCase()}.${name.toUpperCase().replace(/-/g, '_')}` : null; }).filter((reference) => reference && sourceText.includes(reference.split(' ')[1])); const missing = [sourceText.includes('getStartupPreferenceEventDefinitions') ? '' : 'getStartupPreferenceEventDefinitions', /eventBus\.publish\(\s*event\s*,\s*preferences\[name\]\s*\)/.test(sourceText) ? '' : 'eventBus.publish(event, preferences[name])', ...eventFailures].filter(Boolean); return { name: 'settings startup preference events derive from settings and event manifests', status: missing.length === 0 && manualReferences.length === 0 ? 'pass' : 'fail', expected: 'manifest-derived startup preference event loop with valid renderer event payloads', actual: missing.length === 0 ? 'derived' : 'manual-or-missing', missing, extra: manualReferences }; }
...
function createPreloadDefaultInvokeValidatorCheck(sourceText) { const sourceFile = ts.createSourceFile('src/preload/subscription.factory.ts', sourceText, ts.ScriptTarget.Latest, true), target = sourceFile.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === 'createDefaultInvokeMethod'), hasCall = (root, name, args) => { let found = false; const visit = (node) => { if (!found && ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === name && node.arguments.map((arg) => arg.getText(sourceFile)).join(', ') === args) found = true; else ts.forEachChild(node, visit); }; if (root) visit(root); return found; }; let arrowBody = null; for (const statement of target?.body?.statements || []) if (ts.isReturnStatement(statement)) { const returned = unwrappedExpression(statement.expression); if (returned && ts.isArrowFunction(returned) && ts.isBlock(returned.body)) arrowBody = returned.body; } const statements = arrowBody?.statements.map((statement) => statement.getText(sourceFile).replace(/\s+/g, ' ')) || [], forwardedIndex = statements.indexOf('const forwardedArgs = args.slice(0, argumentCount);'), validateIndex = statements.indexOf('const failure = metadata ? validatePreloadInvokeArguments(metadata, forwardedArgs) : null;'), failureIndex = statements.findIndex((statement) => /^if \(failure\) \{.*return Promise\.resolve\(failure\.fallback\);.*\}$/.test(statement)), invokeIndex = statements.indexOf('return ipcRenderer.invoke(channel, ...forwardedArgs);'), missing = [[target, 'createDefaultInvokeMethod'], [hasCall(target?.body, 'requirePreloadInvokeMetadata', 'apiName, methodName, manifestEntry'), 'requirePreloadInvokeMetadata(apiName, methodName, manifestEntry)'], [forwardedIndex !== -1, 'forwardedArgs'], [validateIndex > forwardedIndex, 'validatePreloadInvokeArguments(metadata, forwardedArgs) after forwardedArgs'], [failureIndex > validateIndex, 'fallback return after validation'], [invokeIndex > failureIndex, 'ipcRenderer.invoke after validation fallback']].filter(([pass]) => !pass).map(([, name]) => name); return { name: 'preload default invoke methods apply manifest argument validators before IPC dispatch', status: missing.length === 0 ? 'pass' : 'fail', expected: 'manifest argument validation in generated invoke method before ipcRenderer.invoke', actual: missing.length === 0 ? 'validated-before-dispatch' : 'missing-or-wrong-order', missing, extra: [] }; }
...
function createRendererBridgeEventMetadataCheck(ipcManifest, eventManifest) { const rendererEventByValue = new Map((eventManifest.scopes.find((entry) => entry.scope === 'renderer')?.events || []).map((event) => [event.value, event])), subscriptionByApi = new Map(collectRendererBridgeNamespaces(ipcManifest).map((namespace) => [namespace.apiName, new Map((namespace.subscriptions || []).map((entry) => [derivePublicMethodName(entry), entry]))])), entries = collectRendererBridgeNamespaces(ipcManifest).flatMap((namespace) => Object.entries(namespace.rendererBridge?.eventChannels || {}).map(([method, event]) => [namespace.apiName, method, event, rendererBridgeEventValue(event)])), expected = entries.map(([apiName, method, _event, value]) => `${apiName}.${method} ${value}`), actual = entries.filter(([apiName, method, event, value]) => { const rendererEvent = rendererEventByValue.get(value), subscription = subscriptionByApi.get(apiName)?.get(method); return event.scope === 'renderer' && rendererEvent && subscription && (event.payload === 'none' ? subscription.payload === 'void' && rendererEvent.payload === 'void' : event.payload === 'passthrough' && subscription.payload !== 'void' && normalizePayloadType(rendererEvent.payload) === normalizePayloadType(subscription.payload)); }).map(([apiName, method, _event, value]) => `${apiName}.${method} ${value}`); return compareSortedValues({ name: 'ipc manifest renderer preload bridge event metadata resolves to renderer events', expected, actual }); }
function createRendererBridgePayloadAliasSourceCheck(ipcManifest, sourceText) { const expected = [...new Set(collectRendererBridgeNamespaces(ipcManifest).flatMap((namespace) => Object.entries(namespace.rendererBridge?.eventChannels || {}).flatMap(([method, event]) => event.payload === 'passthrough' ? [...normalizePayloadType((namespace.subscriptions || []).find((entry) => derivePublicMethodName(entry) === method)?.payload || '').matchAll(/\b[A-Z][A-Za-z0-9]*Payload\b/g)].map((match) => match[0]) : [])))].sort(), sourceFile = ts.createSourceFile('src/shared/events/event-payloads.ts', sourceText, ts.ScriptTarget.Latest, true), contractNames = new Set(), localNames = new Set(); for (const statement of sourceFile.statements) { if ((ts.isImportDeclaration(statement) || ts.isExportDeclaration(statement)) && statement.moduleSpecifier && String(statement.moduleSpecifier.getText(sourceFile)).includes('@shared/ipc/preload-api.contract.js') && statement.exportClause?.elements) for (const element of statement.exportClause.elements) contractNames.add(element.name.text); if (ts.isImportDeclaration(statement) && statement.importClause?.namedBindings && ts.isNamedImports(statement.importClause.namedBindings) && String(statement.moduleSpecifier.getText(sourceFile)).includes('@shared/ipc/preload-api.contract.js')) for (const element of statement.importClause.namedBindings.elements) contractNames.add(element.name.text); if (ts.isTypeAliasDeclaration(statement) || ts.isInterfaceDeclaration(statement)) localNames.add(statement.name.text); } return compareSortedValues({ name: 'renderer bridge passthrough payload aliases reuse ipc contract types', expected, actual: expected.filter((name) => contractNames.has(name) && !localNames.has(name)) }); }
...
function buildPhase1DriftReport(manifests = loadManifests(), options = {}) {
  ...
  checks.push(createRendererBridgePayloadAliasSourceCheck(manifests.ipc, eventPayloadsSource));
  checks.push(createExactSourceCheck({
  ...
  checks.push(createSettingsStartupPreferenceEventOwnershipCheck(manifests.settings, manifests.events, readProjectText('src/renderer/application/orchestrators/preferences.orchestrator.ts')));
