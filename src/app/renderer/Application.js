...
class Application {
  ...
  async initialize() {
    ...
    try {
      ...
      // 4. Initialize adapter registry (async initialization)
      await this._initializeAdapterRegistry();
  ...
  async cleanup() {
    ...
    try {
      // Clean up UIController (event listeners, VolumeControl)
   ...
   * @private
   */
  async _initializeAdapterRegistry() {
    try {
      const adapterRegistry = this.container.resolve('adapterRegistry');
