...
import { AppOrchestrator } from '@ui/app.orchestrator.js';
...
import { UIEventHandler } from '@ui/event-handler.js';
...
function createRendererContainer() {
  ...
  container.registerSingleton(
    'visibilityHandler',
    function(loggerFactory) {
      return new VisibilityHandler(loggerFactory.create('VisibilityHandler'));
    },
    ['loggerFactory']
  );
  ...
  // Adapter Registry - Maps devices to their adapters
  ...
  container.registerSingleton(
    ...
    function (deviceService, eventBus, loggerFactory, adapterRegistry, ipcClient) {
      return new StreamingService({ deviceService, eventBus, loggerFactory, adapterRegistry, ipcClient });
    },
  ...
  container.registerSingleton(
    ...
    function (eventBus, uiController, loggerFactory) {
      return new UIEventHandler({ eventBus, uiController, loggerFactory });
    },
  ...
  container.registerSingleton(
    ...
    function (streamingService, appState, uiController, eventBus, loggerFactory, canvasRenderer, viewportManager, visibilityHandler, streamHealthMonitor, gpuRendererService) {
