...
class TrayManager {
   ...
   */
  createTray() {
    // Determine if running from bundled dist or source
    const appPath = app.getAppPath();
    const isRunningFromDist = appPath.includes('/dist/main') || appPath.includes('\\dist\\main');

    // In bundled environment (dist/), assets are in dist/renderer/assets
    // In dev (source), assets are in project root
    const trayIconPath = isRunningFromDist
      ? path.join(appPath, '../renderer/assets/tray-icon.png')
