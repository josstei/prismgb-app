...
export class StorageService {
  ...
  getItem(key) {
    try {
      ...
      console.warn(`StorageService.getItem failed for key "${key}":`, error.message);
      return null;
    }
  }
  ...
  setItem(key, value) {
    try {
      ...
      if (error.name === 'QuotaExceededError' || error.code === 22) {
        console.warn('StorageService: Quota exceeded, attempting cleanup');
        ...
        try {
          ...
          console.error(`StorageService: Quota still exceeded after cleanup for key "${key}"`);
          return false;
        }
      }
      console.error(`StorageService.setItem failed for key "${key}":`, error.message);
      return false;
    }
  }
  ...
  removeItem(key) {
    try {
      ...
      console.warn(`StorageService.removeItem failed for key "${key}":`, error.message);
    }
  }
