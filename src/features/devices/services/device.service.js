...
class DeviceService extends BaseService {
   ...
   * @returns {boolean} True if supported device
   */
   ...
   * Returns first matching supported device
   ...
   * @returns {Promise<Object>} Device enumeration result
   */
   ...
   * @returns {Promise<MediaDeviceInfo|null>} The supported device or null
   */
  async discoverSupportedDevice() {
    ...
    const storedId = this._getStoredDeviceId(null);
    if (storedId && this.hasMediaPermission && this.videoDevices.length > 0) {
      const device = this.videoDevices.find(d => d.deviceId === storedId);
    ...
    // Need permission to get labels - try stored ID first, then first two devices
    const deviceIdsToTry = [
      storedId,
