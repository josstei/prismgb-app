...
import { DeviceDetectionHelper } from '../../devices/shared/device-detection.js';
...
export class StreamingService extends BaseService {
  constructor(dependencies) {
    super(dependencies, ['deviceService', 'eventBus', 'loggerFactory', 'adapterRegistry', 'ipcClient'], 'StreamingService');
  ...
  /**
   * Start streaming
   * @param {string} deviceId - Device ID (optional, will auto-select if not provided)
   * @returns {Promise<Object>} Result with stream and device info
   */
  ...
  async _performStart(deviceId) {
    try {
      ...
      this.currentAdapter = this.adapterRegistry.getAdapterForDevice(device, {
