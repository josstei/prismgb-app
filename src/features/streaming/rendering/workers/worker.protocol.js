...
export function isValidWorkerResponse(response) {
  return (
    ...
    Object.values(WorkerResponseType).includes(response.type)
  );
}

/**
 * Extract transferable objects from a message payload
 * Used for efficient postMessage with zero-copy transfer
 * @param {Object} payload - Message payload
 * @returns {Array} Array of transferable objects
 */
export function getTransferables(payload) {
  const transferables = [];

  if (payload.canvas instanceof OffscreenCanvas) {
    transferables.push(payload.canvas);
  }

  if (payload.imageBitmap instanceof ImageBitmap) {
    transferables.push(payload.imageBitmap);
  }
