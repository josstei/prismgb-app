 ...
 *
 * Presets are designed for different use cases:
 * - AUTHENTIC: True to original GBC display
 * - VIVID: Enhanced colors for modern displays
 * - SHARP: Maximum clarity with edge enhancement
 * - RETRO_CRT: Classic CRT monitor simulation
 * - LCD_GRID: Visible LCD pixel structure
 */
...
export const RenderPresets = Object.freeze({
  /**
   * Authentic - True to original Game Boy Color display
   * No artificial enhancements, just accurate color reproduction
   */
  AUTHENTIC: Object.freeze({
    id: 'authentic',
    name: 'Authentic',
    description: 'True to original GBC display',
    upscale: {
      enabled: true
    },
    unsharp: {
  ...
  /**
   * Vivid - Enhanced colors with boosted saturation
  ...
  SHARP: Object.freeze({
    id: 'sharp',
    name: 'Sharp',
    description: 'Crisp edges, maximum clarity',
    upscale: {
      enabled: true
    },
    unsharp: {
  ...
  /**
   * LCD Grid - Visible LCD pixel structure
   * Simulates the look of the original GBC LCD panel
   */
  LCD_GRID: Object.freeze({
    id: 'lcd-grid',
    name: 'LCD Grid',
 ...
 * Default preset ID
 */
export const DEFAULT_PRESET_ID = 'vivid';
...
export function getPresetIds() {
  return Object.values(RenderPresets).map(preset => preset.id);
}
 ...
 * @returns {boolean} True if valid preset ID
 */
