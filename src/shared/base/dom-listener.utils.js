...
export function createDomListenerManager(options = {}) {
  ...
  return {
    ...
    /**
     * Remove all listeners associated with a specific target element
     * Useful when an element is being replaced (e.g., canvas recreation)
     * @param {EventTarget} targetToRemove - DOM element to remove listeners for
     * @returns {number} Number of listeners removed
     */
    removeByTarget(targetToRemove) {
      let removed = 0;
      for (let i = listeners.length - 1; i >= 0; i--) {
        const { target, event, handler, opts } = listeners[i];
