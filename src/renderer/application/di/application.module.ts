...
export const applicationModule = new ContainerModule(({ bind }) => {
  ...
  bind(TOKENS.preferencesOrchestrator).to(SettingsPreferencesOrchestrator).inSingletonScope();
  bind(TOKENS.displayModeOrchestrator).to(SettingsDisplayModeOrchestrator).inSingletonScope();
  bind(TOKENS.updateOrchestrator).to(UpdateOrchestrator).inSingletonScope();
