...
describe('UIController', () => {
  ...
  beforeEach(() => {
    ...
    mockElements = {
      ...
      volumeSliderContainer: { classList: { contains: vi.fn().mockReturnValue(false) } },
    ...
    mockRegistry = {
      ...
      get: vi.fn((name) => {
        switch (name) {
          ...
          case 'volumeControl': return mockVolumeControl;
  ...
  describe('setVolume', () => {
    it('should delegate to VolumeControl', () => {
      controller.setVolume(0.5);

      expect(mockVolumeControl.setVolume).toHaveBeenCalledWith(0.5);
    });
  });

  describe('toggleVolumeSlider', () => {
    it('should show slider when true', () => {
      controller.toggleVolumeSlider(true);

      expect(mockVolumeControl.showSlider).toHaveBeenCalled();
    });

    it('should hide slider when false', () => {
      controller.toggleVolumeSlider(false);

      expect(mockVolumeControl.hideSlider).toHaveBeenCalled();
    });
  });

  describe('isVolumeSliderVisible', () => {
    it('should return true when container has visible class', () => {
      mockElements.volumeSliderContainer.classList.contains.mockReturnValue(true);

      expect(controller.isVolumeSliderVisible()).toBe(true);
    });

    it('should return false when container does not have visible class', () => {
      mockElements.volumeSliderContainer.classList.contains.mockReturnValue(false);

      expect(controller.isVolumeSliderVisible()).toBe(false);
    });
  });
