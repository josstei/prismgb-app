...
import { UIComponentRegistry } from '@ui/component.registry.js';
...
describe('UIComponentRegistry', () => {
  ...
  beforeEach(() => {
    ...
    mockFactory = {
      ...
      createVolumeControl: vi.fn().mockReturnValue(mockComponents.volumeControl),
    ...
    mockElements = {
      ...
      cinematicBtn: { classList: { add: vi.fn(), remove: vi.fn() } },
      currentResolution: { textContent: '' },
      currentFPS: { textContent: '' },
      screenshotBtn: { disabled: false },
      recordBtn: { disabled: false },
      volumeSlider: { value: '100' },
      volumePercentage: { textContent: '' },
      volumeSliderContainer: { classList: { contains: vi.fn() } },
      streamVideo: { volume: 1 },
      volumeBtn: { addEventListener: vi.fn() }
    };
  ...
  describe('initialize', () => {
    ...
    it('should create VolumeControl with correct elements', () => {
      expect(mockFactory.createVolumeControl).toHaveBeenCalledWith({
        volumeSlider: mockElements.volumeSlider,
        volumePercentage: mockElements.volumePercentage,
        volumeSliderContainer: mockElements.volumeSliderContainer,
        streamVideo: mockElements.streamVideo,
        volumeButton: mockElements.volumeBtn
      });
    });

    it('should store VolumeControl in components Map', () => {
      expect(registry.components.get('volumeControl')).toBe(mockComponents.volumeControl);
    });

    it('should call setupClickOutside on VolumeControl', () => {
      expect(mockComponents.volumeControl.setupClickOutside).toHaveBeenCalled();
    });
  ...
  describe('get', () => {
    ...
    it('should return volumeControl by name', () => {
      const component = registry.get('volumeControl');

      expect(component).toBe(mockComponents.volumeControl);
    });
  ...
  describe('dispose', () => {
    ...
    it('should call dispose on volumeControl', () => {
      registry.dispose();

      expect(mockComponents.volumeControl.dispose).toHaveBeenCalled();
    });
    ...
    it('should log debug message for each component disposed', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('Disposing component: volumeControl');
    });
