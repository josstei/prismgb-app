...
import { ShaderSelectorComponent } from '@/ui/components/shader-selector.js';

// Mock the render presets module
vi.mock('@/features/streaming/rendering/presets/render.presets.js', () => ({
  getPresetsForUI: vi.fn(() => [
    { id: 'authentic', name: 'Authentic', description: 'True to original' },
    { id: 'vivid', name: 'Vivid', description: 'Enhanced colors' },
    { id: 'sharp', name: 'Sharp', description: 'Maximum clarity' }
  ])
}));
...
describe('ShaderSelectorComponent', () => {
  ...
  describe('initialize', () => {
    ...
    it('should load current preset on initialize', () => {
      ...
      expect(component.currentPresetId).toBe('vivid');
    });
    ...
    it('should mark current preset as active', () => {
      ...
      expect(activeOption.dataset.presetId).toBe('vivid');
    });
  ...
  describe('Preset selection', () => {
    ...
    it('should call settings service when selecting preset', () => {
      const option = mockElements.shaderDropdown.querySelector('[data-preset-id="sharp"]');
      option.click();

      expect(mockSettingsService.setRenderPreset).toHaveBeenCalledWith('sharp');
    });
    ...
    it('should hide dropdown after selection', async () => {
      ...
      // Dropdown hides after animation delay
    ...
    it('should not call settings service when selecting same preset', () => {
      const option = mockElements.shaderDropdown.querySelector('[data-preset-id="vivid"]');
      option.click();
  ...
  describe('Click outside', () => {
    ...
    it('should not hide when clicking inside dropdown', () => {
      ...
      insideElement.closest = vi.fn((selector) => {
        if (selector === '.shader-dropdown') return mockElements.shaderDropdown;
        return null;
      });
