...
describe('UIEventHandler', () => {
  ...
  describe('Event Handlers - Volume', () => {
    beforeEach(() => {
      handler.initialize();
    });

    it('should handle ui:volume-level event', () => {
      subscribedHandlers['ui:volume-level']({ level: 75 });

      expect(mockUiController.setVolume).toHaveBeenCalledWith(75);
    });

    it('should handle ui:volume-slider-visible event', () => {
      subscribedHandlers['ui:volume-slider-visible']({ visible: true });

      expect(mockUiController.toggleVolumeSlider).toHaveBeenCalledWith(true);
    });
  });

  describe('Event Handlers - Modes', () => {
    ...
    it('should handle ui:cinematic-mode event', () => {
