...
vi.mock('@ui/components/status-notification.js', () => ({
...
vi.mock('@ui/components/device-status.js', () => ({
...
vi.mock('@ui/components/stream-controls.js', () => ({
...
vi.mock('@features/settings/ui/settings-menu.js', () => ({
...
vi.mock('@ui/components/volume-control.js', () => ({
  VolumeControl: class {
    constructor(config) {
      this.type = 'VolumeControl';
      this.config = config;
    }
  }
}));
...
describe('UIComponentFactory', () => {
  ...
  describe('createVolumeControl', () => {
    it('should create VolumeControl with config', () => {
      const config = { volumeSlider: {}, volumeBtn: {} };

      const component = factory.createVolumeControl(config);

      expect(component.type).toBe('VolumeControl');
      expect(component.config).toBe(config);
    });
  });
});
