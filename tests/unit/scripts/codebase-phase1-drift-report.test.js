...
const windowManifestBridgeCall = /this\.disposables\.replace\(\s*RendererPreloadBridgeDescriptors\.windowAPI\.lifecycleKey,\s*createRendererPreloadEventBridge\(\{[\s\S]*?\}\)\s*\)\s*;/;
const windowBridgeSource = () => fs.readFileSync(windowBridgePath, 'utf8');
const windowBridgeShadowCases = [
  { name: 'local windowAPI binding', prefix: '', setup: 'const windowAPI = null;', apiExpression: 'windowAPI' },
...
describe('codebase phase 1 drift report', () => {
  ...
  it('fails closed when EventChannels source cannot be parsed', () => {
    ...
    expect(eventCheck).toMatchObject({ status: 'fail', expectedCount: 75, actualCount: 0, missing: expect.arrayContaining(['EventChannels.DEVICE.STATUS_CHANGED device:status-changed']) });
  });
  ...
  windowBridgeShadowCases.forEach(({ name, prefix, setup, apiExpression }) => {
    it(`fails when windowAPI manifest bridge wiring uses ${name}`, () => {
      let baseSource = `${prefix}${windowBridgeSource()}`;
      if (name === 'local windowAPI binding') {
        baseSource = replaceOnce(baseSource, 'const windowAPI = window.windowAPI;', 'const windowAPI = null;');
      }
      const sourceWithSetup = setup ? replaceOnce(baseSource, 'if (windowAPI) {\n', `if (windowAPI) {\n      ${setup}\n`) : baseSource;
      const source = apiExpression === 'windowAPI'
        ? sourceWithSetup
        : replaceOnce(sourceWithSetup, 'api: windowAPI,', `api: ${apiExpression},`);
      const { report } = buildPhase1DriftReport(loadManifests(), { rendererBridgeSourceOverrides: { [windowBridgePath]: source } });
      const bridgeCheck = checkNamed(report, 'renderer preload bridge wiring derives subscriptions from ipc manifest');
      expect(report.status).toBe('fail');
      expect(bridgeCheck).toMatchObject({ status: 'fail' });
  ...
  it('fails when manifest bridge api wiring uses a shadowed global object', () => {
    ...
    const bridgeCheck = checkNamed(report, 'renderer preload bridge wiring derives subscriptions from ipc manifest');
    expect(report.status).toBe('fail');
    expect(bridgeCheck.extra).toContain(`${path}: api updateAPI`);
  });
