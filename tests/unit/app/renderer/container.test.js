...
// UI layer mocks
vi.mock('@ui/app.state.js', () => ({
  AppState: vi.fn()
}));

vi.mock('@ui/app.orchestrator.js', () => ({
  AppOrchestrator: vi.fn()
}));

vi.mock('@ui/ui-setup.orchestrator.js', () => ({
  UISetupOrchestrator: vi.fn()
}));

vi.mock('@ui/component.factory.js', () => ({
  UIComponentFactory: vi.fn()
}));

vi.mock('@ui/component.registry.js', () => ({
  UIComponentRegistry: vi.fn()
}));

vi.mock('@ui/effects.js', () => ({
  UIEffects: vi.fn()
}));

vi.mock('@ui/event-handler.js', () => ({
...
vi.mock('@infrastructure/browser/browser-apis.service.js', () => ({
  BrowserAPIsService: vi.fn()
}));
...
describe('Renderer Container', () => {
  ...
  describe('createRendererContainer', () => {
    ...
    it('should register adapterRegistry singleton', () => {
      const container = containerModule.createRendererContainer();
