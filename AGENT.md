...
The Mod Retro Chromatic is a Game Boy-compatible handheld with two key components:

- **FPGA**: Implements cycle-accurate Game Boy/Game Boy Color hardware emulation. Generates the video frames that PrismGB streams.
- **MCU**: Handles USB communication, system management, and acts as the bridge between the FPGA and the host computer. PrismGB communicates with the MCU over USB to receive video data.
...
- No inline comments; only JSDoc for documentation
- Code should be self-documenting through clear naming

## Architectural Philosophy

- **Clean Separation of Concerns**: Every file has a single, well-defined responsibility
- **Future-First Design**: Always design for long-term extensibility with interfaces, abstractions, and loose coupling
- **Strict Contracts**: Enforce rigid typing, explicit interfaces, and consistent architectural standards
- **Maintainability First**: Architecture decisions prioritize testing, enhancement, and organization
- **Dependency Injection**: Loose coupling via constructor injection, never direct instantiation
...
Do NOT include AI attribution (e.g., "Generated with Claude Code", "Co-Authored-By") in commit messages.
